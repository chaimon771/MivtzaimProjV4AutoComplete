...
import example.haim.mivtzaimprojv4.moduls.User;
...
//            Log.d("GAPI-Resume", "Google play services not available - show dialog to ask user to download it");
//            GoogleApiAvailability.getInstance().getErrorDialog(this, response, 1);
//        } else {
//            Log.d("GAPI-Resume", "Google play services is available, no action required");
//        }
    ...
    void sha1() {
        try {
            PackageInfo info = getPackageManager().getPackageInfo("example.haim.mivtzaimprojv4", PackageManager.GET_SIGNATURES);
...
//                                        new AuthUI.IdpConfig.Builder(AuthUI.FACEBOOK_PROVIDER).build()
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if ((response.getErrorCode() == ErrorCodes.UNKNOWN_ERROR))
            if (requestCode == RC_SIGN_IN && resultCode != RESULT_OK) { //Result not Ok - we have a problem to sign in.
